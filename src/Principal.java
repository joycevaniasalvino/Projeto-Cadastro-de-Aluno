...
class No {
  ...
  class Tree {
    ...
    public void inserir(String n, long r) {
      No novo = new No(); // cria um novo Nó
      novo.nome = n;
      novo.rgn = r; // atribui o valor recebido ao item de dados do Nó
      novo.dir = null;
      novo.esq = null;
      ...
      if (root == null) root = novo;
        ...
        while(true) {
          anterior = atual;
          if (r <= atual.rgn) { // ir para esquerda
            atual = atual.esq;
            if (atual == null) {
              anterior.esq = novo;
              return;
    ...
    public No buscar(long chave) {
      ...
      while (atual.rgn != chave) { // enquanto nao encontrou
        if(chave < atual.rgn ) atual = atual.esq; // caminha para esquerda
    ...
    public boolean remover(long r) {
      ...
      // ****** Buscando o valor **********
      while (atual.rgn != r) { // enquanto nao encontrou
        pai = atual;
        if(r < atual.rgn ) { // caminha para esquerda
          atual = atual.esq;
    ...
    public void caminhar() {
      ...
      if (root != null ) {  // se arvore nao esta vazia
         System.out.print("\n Valor minimo: " + min().rgn);
         System.out.println("\n Valor maximo: " + max().rgn);
      }
    }
    ...
    public void inOrder(No atual) {
      if (atual != null) {
        inOrder(atual.esq);
        System.out.print(atual.nome + " " + atual.rgn + " ");
        inOrder(atual.dir);
      }
    }
  ...
  class ArvoreBinariaApp {
    public static void main(String[] args) {
      ...
      switch(opcao) {
          ...
          case 4: {
            if (arv.root == null) {
              //Scanner ler_arquivo = Scanner("Arquivo.txt");
            }
            else {
              arv.caminhar();
            }
